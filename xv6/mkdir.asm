...
strcmp(const char *p, const char *q)
{
  ...
  c1:	84 c0                	test   %al,%al
  c3:	74 22                	je     e7 <strcmp+0x37>
  c5:	38 d8                	cmp    %bl,%al
  c7:	74 0d                	je     d6 <strcmp+0x26>
  c9:	eb 1e                	jmp    e9 <strcmp+0x39>
  cb:	90                   	nop
  cc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  d0:	38 d8                	cmp    %bl,%al
  d2:	89 f1                	mov    %esi,%ecx
  d4:	75 13                	jne    e9 <strcmp+0x39>
    p++, q++;
  d6:	83 c2 01             	add    $0x1,%edx
  while(*p && *p == *q)
  d9:	0f b6 02             	movzbl (%edx),%eax
    p++, q++;
  dc:	8d 71 01             	lea    0x1(%ecx),%esi
  while(*p && *p == *q)
  df:	0f b6 59 01          	movzbl 0x1(%ecx),%ebx
  e3:	84 c0                	test   %al,%al
