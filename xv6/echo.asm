...
strcmp(const char *p, const char *q)
{
  ...
  b1:	84 c0                	test   %al,%al
  b3:	74 22                	je     d7 <strcmp+0x37>
  b5:	38 d8                	cmp    %bl,%al
  b7:	74 0d                	je     c6 <strcmp+0x26>
  b9:	eb 1e                	jmp    d9 <strcmp+0x39>
  bb:	90                   	nop
  bc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  c0:	38 d8                	cmp    %bl,%al
  c2:	89 f1                	mov    %esi,%ecx
  c4:	75 13                	jne    d9 <strcmp+0x39>
    p++, q++;
  c6:	83 c2 01             	add    $0x1,%edx
  while(*p && *p == *q)
  c9:	0f b6 02             	movzbl (%edx),%eax
    p++, q++;
  cc:	8d 71 01             	lea    0x1(%ecx),%esi
  while(*p && *p == *q)
